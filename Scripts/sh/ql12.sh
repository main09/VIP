...
if [ "$access" != "2" ]; then
    if [ "$(grep -c "token" $DATA_PATH/config/auth.json)" != 0 ]; then
        log "8.开始青龙内部配置"
        docker exec -it $CONTAINER_NAME bash -c "$(curl -fsSL https://mirror.ghproxy.com/https://raw.githubusercontent.com/yanyuwangluo/VIP/main/Scripts/sh/1customCDNN.sh)"
    else
